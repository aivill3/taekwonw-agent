...
왜 별도 명령인가:
  CPU에서 로컬 이미지 생성은 장당 수십 초~수 분이다. publish 안에서 돌리면
  초안 3편에 삽화 4장씩이면 30분 넘게 콘솔이 묶인다. 초안 자체는 2분이면
  끝나므로, 글을 먼저 발행해 확인하고 삽화는 시간 될 때 따로 채우는 편이 낫다.

    py run.py publish --no-images     초안만 (빠름)
    py run.py images                  삽화 생성 후 Notion에 삽입 (느림)

  GPU가 있거나 IMAGE_SOURCE=gemini 라면 publish 한 번으로 끝내도 된다.
