...
# 챕터 하나에 지시할 분량의 하한. 목표가 가장 낮은 구간(1,000자)에서
# 고정 부분을 빼면 챕터당 90자 안팎이 되는데, 그러면 기사 한 건을 한두
# 문장으로 끝내라는 지시가 된다. 그보다 짧게는 시키지 않는다.
...
def build_prompt(
    ...
    #
    # 전체 목표에서 고정 부분(도입부·마무리·FAQ)을 먼저 빼고 나눈다.
    # 2026-09-23: 전체를 그대로 챕터 수로 나눠 '한 챕터 425~475자' 로
    # 지시했더니, 모델은 챕터를 그대로 지키고 고정 부분 약 650자를 더해
    # 순수 본문 평균 2,351자(목표 1,700~1,900)가 나왔다. 지시의 산수가
    # 틀렸던 것이다.
    n_ch = max(1, brief.chapter_count)
    if n_ch >= CHAPTER_HINT_MIN:
        per_lo = max(MIN_CHAPTER_CHARS, (target_lo - FIXED_PARTS_CHARS) // n_ch)
        per_hi = max(MIN_CHAPTER_CHARS, (target_hi - FIXED_PARTS_CHARS) // n_ch)
        per_chapter_hint = (
            f"\n  도입부·마무리·자주 묻는 질문을 합쳐 약 {FIXED_PARTS_CHARS:,}자를 쓰고,"
            f"\n  나머지를 챕터 {n_ch}개가 나눕니다. **한 챕터는 {per_lo:,}~{per_hi:,}자**입니다."
            f"\n  한 챕터를 길게 쓰고 다른 챕터를 줄이지 마세요."
        )
    else:
        per_chapter_hint = ""

    # 분량 숫자는 프롬프트 맨 앞에서 한 번 못 박는다.
    #
    # 2026-09-23: 1,700~1,900자로 지시한 묶음 글이 순수 본문 평균 2,351자
    # (오전 3편)·3,135자(오후 1편)로 나왔다.
    # 프롬프트 안에 분량 신호가 세 개 있었다.
    #   [작성 규칙]  가이드 전문의 '1,800~2,100자' 와 '줄바꿈 포함 2,600~3,100자'
    #   [문체 예시]  '문체·구성·분량을 참고하세요' — 그런데 예시는 1,800자에서
    #               잘린 발췌라 원래 길이를 알 수 없다
    #   [출력 형식]  맨 끝의 '1,700~1,900자' (정답은 이것 하나)
    # 가이드 전문이 수천 자라 끝의 지시가 앞의 숫자에 묻힌다. 맨 앞에서
    # 이 글의 숫자를 먼저 주고, 다른 숫자가 보이면 이것을 따르라고 적는다.
    length_lead = (
        f"이 글의 본문 분량은 **{target_lo:,}~{target_hi:,}자**입니다"
        f" (줄바꿈 제외, 챕터 {n_ch}개). 아래 [작성 규칙]이나 [문체 예시]에서"
        f" 다른 분량이 보여도 이 숫자를 따릅니다."
    )
...
────────────────────────────────
[문체 예시] 우리 블로그의 기존 글입니다. 문체와 구성만 참고하세요.
내용을 베끼지 말고 형식만 따르세요. 앞부분만 잘라 넣었으므로
분량은 참고하지 않습니다.
────────────────────────────────
